...
# ************************************************************************
# File:    Be_atom_hf1_imesh.py
# Purpose: Solve the ground-state of beryllium atom in Hartree-Fock model with imported mesh, including exact Fock matrix diagonalization step
...
while True:
    ...
    # step 2 - Fock matrix setup and diagonalization
    mat11=0.5*assemble(inner(grad(frx1s),grad(frx1s))*dx)+assemble(2.0*(v1s+v2s)*frx1s*frx1s*dx)-assemble(v12*frx1s*frx2s*dx)-assemble(v1s*frx1s*frx1s*dx)-assemble(v_ext_corr*frx1s*frx1s*dx)
    mat12=0.5*assemble(inner(grad(frx1s),grad(frx2s))*dx)+assemble(2.0*(v1s+v2s)*frx1s*frx2s*dx)-assemble(v2s*frx1s*frx2s*dx)-assemble(v12*frx1s*frx1s*dx)-assemble(v_ext_corr*frx1s*frx2s*dx)
    mat21=0.5*assemble(inner(grad(frx1s),grad(frx2s))*dx)+assemble(2.0*(v1s+v2s)*frx1s*frx2s*dx)-assemble(v1s*frx2s*frx1s*dx)-assemble(v12*frx2s*frx2s*dx)-assemble(v_ext_corr*frx1s*frx2s*dx)
    mat22=0.5*assemble(inner(grad(frx2s),grad(frx2s))*dx)+assemble(2.0*(v1s+v2s)*frx2s*frx2s*dx)-assemble(v12*frx1s*frx2s*dx)-assemble(v2s*frx2s*frx2s*dx)-assemble(v_ext_corr*frx2s*frx2s*dx)

    r1s_p=r1s
    r2s_p=r2s

    mat=np.array([[mat11,mat12],[mat21,mat22]])
    ev,ef=np.linalg.eig(mat)
    ind1=0
    ind2=1
    if ev[0]>ev[1]:
        ind1=1
