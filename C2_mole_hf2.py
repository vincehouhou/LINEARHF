...
# ************************************************************************
# File:    C2_mole_hf2.py
# Purpose: Solve the ground-state of C2 molecule in Hartree-Fock model, including energy correction step
...
while abs(energy-energy_p)>threshold:
    ...
    if abs(energy-energy_p)<threshold2:
        ...
        for i in range(noe):
            ...
            tmp1.vector()[:]=rx_p[i]
            tmp2.vector()[:]=rx[i]
            t[i]+=assemble(inner(v_ext,tmp1-tmp2)*dx)

            for j in range(noe):
                t[i]+=assemble(inner(-2.0*vpof[findex(j,j)]*tmp1+vpof[findex(j,i)]*frx[j],tmp1-tmp2)*dx)
        
            n[i]=assemble(tmp2*tmp2*dx)
            r[i]+=t[i]/n[i]
            print t[i],n[i],t[i]/n[i],assemble(inner(tmp1-tmp2,tmp1-tmp2)*dx)
