...
# ************************************************************************
# File:    H2_mole_rlsda.py
# Purpose: Solve the ground-state of H2 molecule in restricted local spin density approximation withour correlation energy, including energy correction step
...
while True:
    ...
    helb1s=(-2.0*v1s*frx1s+v_ext_corr*frx1s-v_xc*frx1s)*v*dx
    ...
    t1=assemble(inner(-2.0*v1s*frx1s+v_ext_corr*frx1s-v_xc*frx1s,frx1s-tmp1)*dx)
    ...
    energy_H=2.0*assemble(v1s*frx1s*frx1s*dx)
    energy_ex=-2.0*assemble(v_ext_corr*frx1s*frx1s*dx)
    energy_xc=3.0/2.0*assemble(v_xc*frx1s*frx1s*dx)
    ...
    print r1s,energy_T,energy_H,energy_ex,energy_xc,energy_nuc,energy_p,energy,energy-energy_p,(time1-time0).total_seconds() 
